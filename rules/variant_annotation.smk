...
rule rsid_annotation:
    ...
    input:
        ...
        os.path.join(config.get("files_path").get("base_joint_call_path"),config.get("rules").get("recal_pass_filter").get("out_dir"),"/all.{interval_name,call_intervals}.PASS.vcf.gz"),
